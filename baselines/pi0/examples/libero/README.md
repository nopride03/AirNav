...
This example runs the LIBERO benchmark: [REDACTED_URL]

Note: When updating requirements.txt in this directory, there is an additional flag `--extra-index-url [REDACTED_URL] that must be added to the `uv pip compile` command.
...
uv pip sync examples/libero/requirements.txt third_party/libero/requirements.txt --extra-index-url [REDACTED_URL] --index-strategy=unsafe-best-match
